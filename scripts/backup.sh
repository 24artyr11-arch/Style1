...
docker compose exec -T bot python -c \
    'import os,sqlite3,sys; path=os.path.join(os.environ.get("DATA_DIR", "/data"), "wardrobe.sqlite3"); assert os.path.isfile(path), f"База не найдена: {path}"; source=sqlite3.connect(path); target=sqlite3.connect(sys.argv[1]); source.backup(target); target.close(); source.close()' \
    "$inside"
